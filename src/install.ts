    ...
    return (done: (error?: ExecException) => void): void => {
        ...
        exec(
            ...
            (error, stdout, stderr) => {
                ...
                const powerShellModules = require("glob").sync(
                    path.join(project.directory, "node_modules", "**", "*.psm1"),
                );
                ...
                if (powerShellModules.length > 0) {
                    ...
                    if (task.execution.PowerShell3) {
                        ...
                        for (const modulePath of powerShellModules) {
                            ...
                            fs.copySync(powerShellModuleDirName, path.join(psModulesPath, powerShellModuleFolderName), {
                                overwrite: true,
                                dereference: true,
                            });
